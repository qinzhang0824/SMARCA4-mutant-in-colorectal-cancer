...
all <-read.table('simceredx_3273s_clinical_info_addSMARCA4_addTMB_addMSI_addAgeGroup.xls',sep='\t',header=T)
all_msk <-read.table('msk_chord_2024_clinical_data_colon_5543s.txt',sep='\t',header=T)
msk_smarca4_clin <-read.table('msk_chord_2024_clinical_data_colon_SMARCA4.mut.324s.txt',sep='\t',header=T)
################################################################# SMARCA4 MSI-H  and MSS
...
plot_data_bi <- res_sig %>%
  ...
  pivot_longer(cols = c(freq_sim, freq_msk),
               ...
               values_to = "Frequency") %>%
  mutate(
    Frequency_plot = ifelse(Cohort == "freq_sim", Frequency, -Frequency),  
...
y_label_pos <- y_max + 25          
y_msk_label_pos <- -y_min - 10      
...
p <- p + 
  geom_text(data = plot_data_bi,
            aes(x = Hugo_Symbol, y = Frequency_plot, label = label_display,
                vjust = ifelse(Frequency_plot > 0, -0.5, 1.5)),  
...
y_max <- max(plot_data_bi$Frequency, na.rm = TRUE)
y_min <- max(plot_data_bi$Frequency, na.rm = TRUE)
y_label_pos <- y_max + 10         
y_msk_label_pos <- -y_min - 5      
